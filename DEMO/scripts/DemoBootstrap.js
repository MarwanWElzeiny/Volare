...
import { resolveAssetUrl } from '../../SDK/Utils/assetPath.js';
...
function initVolare(path = null, options = {}) {
  setHdriBasePath(resolveAssetUrl('./models/HDRI/'));
  ...
  if (selectedModelPath) {
    const resolvedModelPath = resolveAssetUrl(selectedModelPath);
    viewer.loadModel(resolvedModelPath).then(() => {
      viewer.centerCameraOnModel();
      viewer.materialManager?.applyOriginalMaterials(viewer.currentModel);

      if (selectedHdriPath) {
        viewer.setEnvironment(resolveAssetUrl(selectedHdriPath));
      } else {
